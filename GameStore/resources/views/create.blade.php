 ...
 <div class="container pt-5 pb-4 d-flex justify-content-center ">
   <div class="row pt-3 pb-3" style="background-color: white;border-radius: 10px;">
    <div class=" mt-5 mb-5 mr-5 ml-5 ">
      ...
      <input type="text" name="denumire" class="form-control input-group"   value="{{old('denumire')}}" required>
    	 @error('denumire')
      ...
      <input type="text" name="pret" class="form-control input-group"    value="{{old('pret')}}" required>
     @error('pret')
            <div class="alert alert-danger mt-2" >
                <strong>{{ $message }}</strong>
             </div>
      ...
      <input type="number" name="min" class="form-control input-group"  value="{{old('min')}}" required>
      @error('min')
            <div class="alert alert-danger mt-2" >
                <strong>{{ $message }}</strong>
             </div>
      ...
      <input type="number" name="max" class="form-control input-group"  value="{{old('max')}}" required>
      @error('max')
            <div class="alert alert-danger mt-2" >
                <strong>{{ $message }}</strong>
             </div>
      ...
      <textarea  name="descriere" class="form-control input-group"  value="{{old('descriere')}}" required>{{old('descriere')}}</textarea>
    @error('descriere')
            <div class="alert alert-danger mt-2" >
                <strong>{{ $message }}</strong>
             </div>
      ...
      <select  name="categorieID" class="form-control input-group"  required>
         @if($categorii)
                    @foreach($categorii as $categorie)
                      <option value='{{$categorie->id}}' {{ (old('categorieID')==$categorie->id? "selected":"") }} >Jocuri de {{$categorie->nume}}</option>
                    @endforeach
         @endif
      </select>
     @error('categorieID')
